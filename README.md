...
**Nachrichten an andere Personen weiterleiten**: Sag „schick Mama: bin gleich da" (oder `/schick Mama: bin gleich da`), und der Bot schickt diese Nachricht an Mama weiter — vorausgesetzt, Mama hat dem Bot selbst schon mal irgendetwas geschrieben (der Bot merkt sich dann automatisch ihren Namen). Ein echtes "Kaltanschreiben" fremder Personen ist bei Telegram-Bots technisch nicht möglich (feste Anti-Spam-Regel von Telegram) — der Bot darf nur Chats antworten, die zuerst selbst geschrieben haben. Schreibt Mama dem Bot danach etwas zurück, bekommst du das automatisch als Telegram-Nachricht weitergeleitet — Mama selbst bekommt dabei keine KI-Antwort vom Bot, nur du erfährst von ihrer Antwort. Schreibt eine neue Person den Bot zum ersten Mal an (Telegram schickt dabei automatisch `/start`), bekommt sie einmalig eine kurze Begrüßung statt gar keiner Antwort — damit der Bot nicht kaputt wirkt.
