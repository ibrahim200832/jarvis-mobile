...
**In einer Gruppe nutzen**: Standardmäßig antwortet der Bot nur im privaten Chat. Damit er auch in einer **Telegram-Gruppe** mitschreiben kann: (1) Bot zur Gruppe hinzufügen, (2) bei [@BotFather](https://t.me/BotFather) `/setprivacy` → deinen Bot auswählen → **Disable** wählen (ohne diesen Schritt sieht der Bot in Gruppen nur direkte Befehle/Erwähnungen, keine normalen Nachrichten — eine feste Telegram-Einschränkung). Danach schaltet sich die Gruppe **automatisch** frei, sobald der Bot dort die erste Nachricht sieht (egal von wem, egal ob `/gruppe`, ein anderer Befehl oder eine `@Erwähnung`) — mit einer kurzen Bestätigung als Antwort. Ab der nächsten Nachricht antwortet der Bot dort jedem Gruppenmitglied ganz normal. `/gruppe` funktioniert weiterhin auch explizit, falls gewünscht.
