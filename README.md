...
**⚠️ Achtung, jeder kann den Bot per `/start` voll freischalten**: Schreibt irgendjemand deinen Bot zum ersten Mal an, schickt Telegram automatisch `/start` — und das schaltet diese Person danach **vollständig** frei, genau wie dich selbst. Sie kann dann ganz normal mit der KI chatten, aber auch deinen Google Kalender bearbeiten/löschen, Anrufe bei deinen Kontakten auslösen, deine Philips-Hue-Lichter oder Home-Connect-Geräte steuern und Videos in deinem Namen hochladen (falls jeweils verbunden). Das ist eine bewusste Einstellung — wer sich das nicht wünscht, sollte den Bot-Namen nicht öffentlich teilen und den Bot-Link geheim halten, oder in `worker/ai-proxy.js` bei der `/start`-Freischaltung eine Bestätigung durch den Besitzer verlangen, statt automatisch freizuschalten.
