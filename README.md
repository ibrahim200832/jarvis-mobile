...
2. **KV-Speicher einrichten**, falls noch nicht geschehen (siehe „Kalender-Erinnerungsanrufe" oben, Schritt „KV-Speicher anlegen") — dort wird u. a. deine eigene Chat-ID gespeichert.
...
**In einer Gruppe nutzen**: Standardmäßig antwortet der Bot nur im privaten Chat. Damit er auch in einer **Telegram-Gruppe** mitschreiben kann: (1) Bot zur Gruppe hinzufügen, (2) bei [@BotFather](https://t.me/BotFather) `/setprivacy` → deinen Bot auswählen → **Disable** wählen (ohne diesen Schritt sieht der Bot in Gruppen nur direkte Befehle/Erwähnungen, keine normalen Nachrichten — eine feste Telegram-Einschränkung). Danach schaltet sich die Gruppe **automatisch** frei, sobald der Bot dort die erste Nachricht sieht (egal von wem, egal ob `/gruppe`, ein anderer Befehl oder eine `@Erwähnung`) — mit einer kurzen Bestätigung als Antwort. `/gruppe` funktioniert weiterhin auch explizit, falls gewünscht. Damit sich der Bot nicht in jedes Gespräch einmischt, antwortet er in einer Gruppe **nur**, wenn er direkt per `@BotName` erwähnt, auf seine eigene Nachricht geantwortet, oder ein Slash-Befehl (z. B. `/suche`, `/termine`, `/hilfe`) an ihn geschickt wird — normale Unterhaltungen zwischen Gruppenmitgliedern ignoriert er einfach. Alle Slash-Befehle funktionieren also auch in Gruppen ganz normal.

**⚠️ Achtung, jede erste Nachricht schaltet den Bot für diese Person voll frei**: Schreibt irgendjemand deinen Bot zum allerersten Mal an — egal ob mit `/start`, normalem Text oder direkt einer Sprachnachricht — wird diese Person sofort **vollständig** freigeschaltet, genau wie du selbst. Sie kann dann ganz normal mit der KI chatten (inklusive Sprachnachrichten senden/empfangen), aber auch deinen Google Kalender bearbeiten/löschen, Anrufe bei deinen Kontakten auslösen, deine Philips-Hue-Lichter oder Home-Connect-Geräte steuern und Videos in deinem Namen hochladen (falls jeweils verbunden). Das ist eine bewusste Einstellung — wer sich das nicht wünscht, sollte den Bot-Namen nicht öffentlich teilen und den Bot-Link geheim halten, oder in `worker/ai-proxy.js` bei der automatischen Freischaltung eine Bestätigung durch den Besitzer verlangen, statt sofort freizuschalten.

**Nachrichten an bekannte, noch nicht freigeschaltete Kontakte weiterleiten**: Sag „schick Mama: bin gleich da" (oder `/schick Mama: bin gleich da`), und der Bot schickt diese Nachricht an Mama weiter — vorausgesetzt, Mama hat dem Bot selbst schon mal irgendetwas geschrieben, aber (noch) kein `/start` gesendet (der Bot merkt sich ihren Namen trotzdem automatisch). Ein echtes "Kaltanschreiben" fremder Personen ist bei Telegram-Bots technisch nicht möglich (feste Anti-Spam-Regel von Telegram) — der Bot darf nur Chats antworten, die zuerst selbst geschrieben haben. Schreibt Mama dem Bot danach etwas zurück, bekommst du das automatisch als Telegram-Nachricht weitergeleitet — Mama selbst bekommt dabei keine KI-Antwort vom Bot, nur du erfährst von ihrer Antwort.
...
Zusätzlich gibt es ein **dauerhaftes Gedächtnis** (kein Zeitlimit, anders als der Gesprächsverlauf oben): Sag „merk dir: \<Fakt\>" (z. B. „merk dir: ich mag keinen Kaffee"), und der Bot berücksichtigt das ab sofort bei jeder künftigen Antwort — auch nach Tagen oder Wochen noch. „meine erinnerungen" zeigt alles Gemerkte an, „vergiss alles" löscht es wieder. Dieses Gedächtnis ist geteilt (alle freigeschalteten Nutzer greifen auf dasselbe JARVIS-Wissen zu) — bringt also jemand anderes JARVIS über `/schick`/`/gruppe`/eigenen Zugang etwas bei, bekommst du als Besitzer eine kurze Telegram-Meldung, was gelernt wurde. Offensichtliche Beleidigungen werden dabei weder gespeichert noch gemeldet, sondern grob herausgefiltert.
...
Der Bot kann auf jede Nachricht (egal ob du tippst oder eine Sprachnachricht schickst) zusätzlich zur Textantwort auch mit einer **eigenen Sprachnachricht** antworten. Das braucht einen kleinen kostenpflichtigen Zusatzdienst (ElevenLabs), weil die kostenlose Cloudflare-KI leider kein Deutsch spricht — die Kosten sind aber sehr gering (Bruchteile eines Cents pro Antwort). Das gilt für dich genauso wie für jeden anderen freigeschalteten Nutzer — alle teilen sich dasselbe ElevenLabs-Kontingent. Schlägt die Sprachantwort bei jemand anderem fehl (z. B. weil das monatliche Kontingent aufgebraucht ist), bekommst du als Besitzer automatisch eine Warnung darüber, die eigentliche Textantwort kommt trotzdem immer an. Damit das (kostenlose) Kontingent bei mehreren Nutzern länger reicht, wird nur der Anfang einer langen Antwort gesprochen (die ersten ca. 300 Zeichen, am Satzende abgeschnitten) — der volle Text kommt trotzdem immer komplett als Textnachricht. Ist das ElevenLabs-Kontingent mal aufgebraucht, springt automatisch ein kostenloser Ersatz ein (dieselbe Sprachausgabe wie translate.google.com, kein eigener Account nötig) — klingt etwas roboterhafter, sorgt aber dafür, dass trotzdem eine Sprachantwort ankommt.
