...
void main() {
  testWidgets('ScanlineOverlay paints without throwing and can be disposed', (tester) async {
    await tester.pumpWidget(
      const MaterialApp(home: SizedBox(width: 300, height: 300, child: ScanlineOverlay())),
    );
    await tester.pump();
  ...
  testWidgets('AccessDeniedFlash shows its text when visible', (tester) async {
    await tester.pumpWidget(const MaterialApp(home: AccessDeniedFlash(visible: true)));
    await tester.pump();
  ...
  testWidgets('AccessDeniedFlash is present but invisible when not visible', (tester) async {
    await tester.pumpWidget(const MaterialApp(home: AccessDeniedFlash(visible: false)));
    await tester.pump();
